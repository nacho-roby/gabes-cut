(function () {
  ...
  function estimateRevenue(sales, basePrice, {
    applyVAT = false,
    averageDiscount = DEDUCTIONS.averageDiscount,
    refunds = DEDUCTIONS.refunds,
    skipRegionalPricing = false,
  } = {}) {
    const gross = sales * basePrice;
    const steps = [
      { label: 'Average discounts',   rate: averageDiscount },
      { label: 'Refunds & returns',   rate: refunds },
    ];
    if (!skipRegionalPricing) {
      steps.push({ label: 'Regional pricing', rate: DEDUCTIONS.regionalPricing });
    }
    steps.push({ label: "Gabe's Cut", rate: DEDUCTIONS.steamCut });
